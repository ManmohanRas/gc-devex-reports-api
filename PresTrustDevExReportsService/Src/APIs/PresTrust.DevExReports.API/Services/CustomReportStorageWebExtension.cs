...
using PresTrust.DevExReports.API.PredefinedReports;
using System;
...
namespace PresTrust.DevExReports.API.Services
{
    public class CustomReportStorageWebExtension : ReportStorageWebExtension
    {
        ...
        {
            ReportDirectory = Path.Combine(env.ContentRootPath, "DevExReports");
        ...
        public override byte[] GetData(string url)
        {
            ...
            try
            {
                //blob url change seperating parameter 
                ...
                if (report != null)
                {
                    ...
                    foreach (string parameterName in parameters.AllKeys)
                    {
                        try
                        {
                            if (report.Parameters[parameterName] != null && report.Parameters[parameterName].Value != null)
                            {
                ...
                //{
                //    var pathrpt = File.ReadAllBytes(Path.Combine(ReportDirectory, url + FileExtension));
            ...
            {
                throw new DevExpress.XtraReports.Web.ClientControls.FaultException(string.Format("Could not find report '{0}'." + " Path:" + ReportDirectory + " ex:" + ex.ToString(), url));
            }
        }
